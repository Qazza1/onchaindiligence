...
app.post('/attest', rateLimit, requireInternalAttestationAuth, async (c) => {
  ...
  if (rawPurpose !== undefined) {
    const allowedPurposes: unknown[] = [ATTESTATION_PURPOSE, ATTESTATION_RECEIPT_PURPOSE, ATTESTATION_ALLOWANCE_PURPOSE, 'swap-action']
    if (!allowedPurposes.includes(rawPurpose)) {
      return c.json({ error: `purpose must be one of: ${allowedPurposes.join(', ')}` }, 400)
    }
  }
  const purpose = rawPurpose as typeof ATTESTATION_PURPOSE | typeof ATTESTATION_RECEIPT_PURPOSE | typeof ATTESTATION_ALLOWANCE_PURPOSE | 'swap-action' | undefined
