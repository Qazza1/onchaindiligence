...
| OD-007 | Standalone MCP verdict logic has diverged from the HTTP verdict | MCP, API | FIXED LOCALLY | API owns one canonical evaluator used by public and authenticated internal routes; MCP delegates and returns the signed envelope unchanged; policy and transport contract tests pass; deploy both services and verify the production unpaid/readiness paths |
